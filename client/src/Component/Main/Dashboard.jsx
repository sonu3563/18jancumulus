...
const Dashboard = ({ folderId = 1, onFolderSelect, searchQuery }) => {
  ...
  const handleDrop = (e) => {
    ...
    handleFileUpload(files); // Proceed if all files are valid
  };
  ...
  const handleFileUpload = (input) => {
    ...
    if (files && files.length > 0) {
      ...
      fileArray.forEach((file) => {
        const customName = editFile?.name || file.name; // Use custom name if editing, otherwise use the original name
        ...
        // Add each file to the files state
        setFiles((prevFiles) => [
          ...prevFiles,
  ...
  const renderOverlay = () => {
    ...
    return (
      <div
        className="fixed top-0 pt-10 left-0 w-[100vw] min-h-screen bg-[rgba(0,0,0,0.8)] text-white z-[1000] flex justify-center items-center"
      >
        ...
        <div
          className=" text-black p-5 rounded-lg max-w-[90%] max-h-[70%] md:min-h-[90%]  w-[80%]"
          style={{
        ...
        <div className="flex border-b-2 md:border-0 mb-0 justify-between items-center mt-2 ml-4 md:px-0 border-gray-300">
          ...
          {!(folderId === "0" || folderId === 1) ? (
            ...
            >
              ...
              <p className="text-md md:text-lg font-semibold text-white rounded-md py-2 px-2">
                Share Folder
              </p>
            </div>
          ) : null}
          ...
          <div className="bg-white p-6 mx-2 rounded shadow-lg max-w-lg w-full">
                ...
                >
                  <div className="w-3/4">
                    <p className="font-medium">
                      {editing ? inputFileName : file.name}
                    </p>
            ...
            <div className="flex justify-between gap-2 my-2">
              <div className="flex gap-x-6">
                ...
                <div className="flex border border-slate-500 rounded-lg p-1 md:p-2 text-xs md:text-sm font-semibold  cursor-pointer text-gray-500">
                  <img src={dropbox} className="h-6 w-6 mr-2 mt-1 ml-1" />
                  <h1 className="mt-1 md:mt-0 md:inline hidden" >Dropbox</h1>
                </div></div>
              <button
                onClick={() => clear()}
