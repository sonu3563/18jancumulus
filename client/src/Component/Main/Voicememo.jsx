...
const saveRecording = async () => {
    ...
    try {
        ...
        // Convert used storage and total available storage to bytes for comparison
        const usedStorageInBytes = folderSize.value * (folderSize.unit === 'KB' ? 1024 : 
                                                       folderSize.unit === 'MB' ? 1024 * 1024 : 
                                                       folderSize.unit === 'GB' ? 1024 * 1024 * 1024 : 1);
        const totalStorageInBytes = storageData ? parseInt(storageData) : 
                                    storageData1 ? parseInt(storageData1) * 1024 * 1024 * 1024 : null;
        const uploadedFileSizeInBytes = audioBlob.size; // Get the size of the audio file

        // console.log(`Used storage in bytes: ${usedStorageInBytes}`);
        // console.log(`Uploaded file size in bytes: ${uploadedFileSizeInBytes}`);
        // console.log(`Total available storage in bytes: ${totalStorageInBytes}`);

        // Check if total storage is null or undefined
        if (!totalStorageInBytes) {
            showAlert("error", "Storage Error", "Total storage information is missing.");
            return;
        }

        // Check if storage limit is exceeded
        if (usedStorageInBytes + uploadedFileSizeInBytes > totalStorageInBytes) {
            showAlert("error", "Storage Limit Exceeded", "You do not have enough available storage.");
            return;
        }
        ...
        if (response.status === 200 || response.status === 201) {
            ...
            fetchAudioFiles();
            showAlert("success", "Upload Successful", "Your voice memo has been uploaded.");
        }
  ...
  return (
    <div className="mt-1 p-2 sm:p-4 max-h-screen bg-white overflow-hidden">
      ...
      <div className="flex flex-col ">
        <h1 className="text-lg md:text-2xl font-normal text-[#1F1F1F]">Your Voice Memo</h1>
        <div
      ...
      <div className=" block mt-2">
        <h2 className="text-lg font-bold">Voices Library</h2>
      </div>
        ...
        {filteredMobileFiles.map((file, index) => (
          ...
          >
            ...
            <div className="flex justify-between">
              <div className="flex items-center gap-2">
                ...
                {editsMode === file._id ? (
                  ...
                  <h3 className="font-bold text-lg">{file.voice_name}</h3>
                )}
              </div>
            ...
            <div className="flex justify-around mt-3">
              ...
              <p className="text-xs text-gray-700  mt-1">{file.duration} sec</p>
            </div>
          </div>
        ))}
      </div>
