...
const ManageUsers = () => {
    ...
    fetch(`${API_URL}/api/auth/get-all-users-data`)
      ...
      .then((data) => {
        if (data.user) {
          ...
          const sortedUsers = data.user.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
          ...
          <table className="w-full border-collapse bg-white shadow-md rounded-md">
            <thead>
              <tr className="bg-blue-500 text-white sticky top-0">
                ...
                <th className="p-3 text-left">Active Membership</th>
              </tr>
            </thead>
            <tbody>
  ...
  user.memberships[user.memberships.length - 1].subscription_id.subscription_name : 'No Membership'}
                    ...
                    <td className="p-3">
                      <span
                        className={`px-3 py-1 rounded-full text-white ${
                          user.activeMembership ? "bg-green-500" : "bg-red-500"
                        }`}
                      >
                        {user.activeMembership ? "Active" : "Not Active"}
                      </span>
                    </td>
                  </tr>
                ))
