...
router.post(
  ...
  async (req, res) => {
    ...
    if (!designeeEmail || !designeeName || !designeePhone) {
      return res
        .status(400)
        .json({ message: "Designee name, phone, and email are required." });
    }
    ...
    try {
      ...
      if (designee) {
        // If the designee is already linked to the user
        if (designee.from_user_id.includes(user_id)) {
          console.log(
            `Designee with email ${designeeEmail} is already linked to user ${user_id}.`
          );
          return res.status(400).json({
            message: "Designee already exists and is linked to your account.",
          });
        } else {
          // If the designee exists but is not linked to this user
          console.log(
            `Designee with email ${designeeEmail} exists but is not linked to user ${user_id}. Linking now.`
          );
          designee.from_user_id.push(user_id);

          if (file) {
            const aws_file_link = await uploadProfilePicture(file); // Helper function to handle AWS upload
            designee.profile = aws_file_link; // Update profile picture if a new one is uploaded
          }

          await designee.save();
          return res.status(200).json({
            message: "Designee linked successfully to your account.",
            designee,
          });
        }
      }

      // If the designee doesn't exist, create a new one
      let otp = crypto
        .randomBytes(5)
        .toString("base64")
        .replace(/[^a-zA-Z0-9]/g, "")
      ...
      if (!emailResponse.success) {
        console.error(
          `Error sending OTP email to ${designeeEmail}:`,
          emailResponse.error
        );
        return res
          .status(500)
          .json({ message: "Error sending OTP email.", error: emailResponse.error });
      }
      ...
      // Upload profile picture to AWS if provided
      if (file) {
        const aws_file_link = await uploadProfilePicture(file);
        designeeData.profile = aws_file_link;
      }

      const newDesignee = new Designee(designeeData);
      await newDesignee.save();

      console.log(`New designee created: ${designeeEmail}`);
      return res.status(200).json({
        message: "Designee created successfully. OTP sent.",
        previewURL: emailResponse.previewURL,
      });
...
// Helper function to handle profile picture upload
async function uploadProfilePicture(file) {
  ...
  return encryptField(aws_file_link); // Assuming this is a function you have that encrypts the link.
}
