...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // If only email is provided, return user existence status
    if (!password) {
      return res.status(200).json({ message: "User exists." });
    }

    // Ensure the stored user password is defined and a string
    if (!user.password || typeof user.password !== 'string') {
      return res.status(500).json({ message: "User password is not properly set." });
    }
